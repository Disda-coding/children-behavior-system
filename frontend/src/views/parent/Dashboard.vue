      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between h-16">
          ...
          <div class="flex items-center space-x-4">
            <span class="text-gray-600">欢迎，{{ authStore.user?.displayName }}</span>
            <button
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm card-hover">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-3xl font-bold text-primary-600 mt-2">{{ familyMembers.length }}</p>
            </div>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm card-hover">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-3xl font-bold text-success-600 mt-2">{{ pointRulesCount }}</p>
            </div>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm card-hover">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-3xl font-bold text-warning-600 mt-2">{{ achievementsCount }}</p>
            </div>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm card-hover">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-3xl font-bold text-danger-600 mt-2">{{ pendingAppealsCount }}</p>
            </div>
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        >
          ...
          <h3 class="font-medium text-gray-800">成就管理</h3>
        </router-link>
        ...
        >
          ...
          <h3 class="font-medium text-gray-800">奖励管理</h3>
        </router-link>
...
const fetchFamilyMembers = async () => {
  try {
    if (authStore.user?.familyId) {
      const response = await familyApi.getFamily(authStore.user.familyId) as { members: any[] };
...
const fetchStats = async () => {
  try {
    // 获取积分规则数量
    const rulesResponse = await pointApi.getRules() as { rules: any[] };
    pointRulesCount.value = rulesResponse.rules.length;

    // 获取成就数量
    const achievementsResponse = await achievementApi.getAchievements() as { achievements: any[] };
    achievementsCount.value = achievementsResponse.achievements.length;

    // 获取待处理申诉数量
    const appealsResponse = await appealApi.getAppeals({ status: 'pending' }) as { appeals: any[] };
    pendingAppealsCount.value = appealsResponse.appeals.length;
  } catch (error) {
